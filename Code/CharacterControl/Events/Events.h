...
struct Event_CreateVampireNPC : public PE::Events::Event_CREATE_MESH
{
	PE_DECLARE_CLASS(Event_CreateVampireNPC);

	Event_CreateVampireNPC(int& threadOwnershipMask) : PE::Events::Event_CREATE_MESH(threadOwnershipMask) {}
	// override SetLuaFunctions() since we are adding custom Lua interface
	static void SetLuaFunctions(PE::Components::LuaEnvironment* pLuaEnv, lua_State* luaVM);
