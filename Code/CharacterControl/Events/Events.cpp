...
PE_IMPLEMENT_CLASS1(Event_CreateVampireNPC, PE::Events::Event_CREATE_SKELETON);

void Event_CreateVampireNPC::SetLuaFunctions(PE::Components::LuaEnvironment* pLuaEnv, lua_State* luaVM)
{
	static const struct luaL_Reg l_Event_CreateVampireNPC[] = {
		{"Construct", l_Construct},
		{NULL, NULL} // sentinel
	};

	// register the functions in current lua table which is the table for Event_CreateSoldierNPC
	luaL_register(luaVM, 0, l_Event_CreateVampireNPC);
}

int Event_CreateVampireNPC::l_Construct(lua_State* luaVM)
{
	PE::Handle h("EVENT", sizeof(Event_CreateVampireNPC));
	...
	// this function should only be called frm game thread, so we can use game thread thread owenrship mask
	Event_CreateVampireNPC* pEvt = new(h) Event_CreateVampireNPC(pContext->m_gameThreadThreadOwnershipMask);
