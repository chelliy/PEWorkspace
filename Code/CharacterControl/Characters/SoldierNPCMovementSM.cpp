...
: m_targetPosition(targetPos), m_runniung(false), m_takeCover(false), m_highCover(false), m_jumpStart(false), m_jumpEnd(false),m_preStartDistance(0)
{ }
...
//	m_targetPostion = pRealEvt->m_targetPosition;
//
//	OutputDebugStringA("PE: PROGRESS: SoldierNPCMovementSM::do_SoldierNPCMovementSM_Event_STAND_SHOOT(): receuved event");
	...
	if (m_state == WALKING_TO_TARGET || m_state == RUNNING_TO_TARGET || m_state == IN_AIR)
	{
		...
		if (pSceneNode)
		{
			...
			if (!preStartActived) {
				...
				if (m_takeCover || m_jumpStart || m_jumpEnd) {
