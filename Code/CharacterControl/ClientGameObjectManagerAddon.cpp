...
void ClientGameObjectManagerAddon::addDefaultComponents()
{
	...
	PE_REGISTER_EVENT_HANDLER(Event_CreateVampireNPC, ClientGameObjectManagerAddon::do_CreateVampireNPC);
...
void ClientGameObjectManagerAddon::do_CreateVampireNPC(PE::Events::Event* pEvt)
{
	assert(pEvt->isInstanceOf<Event_CreateVampireNPC>());

	Event_CreateVampireNPC* pTrueEvent = (Event_CreateVampireNPC*)(pEvt);

	createVampireNPC(pTrueEvent);
}

void ClientGameObjectManagerAddon::createVampireNPC(Event_CreateVampireNPC* pTrueEvent)
{
	PEINFO("CharacterControl: GameObjectManagerAddon: Creating createVampireNPC\n");

	PE::Handle hVampireNPC("SoldierNPC", sizeof(VampireNPC));
	VampireNPC* pVampireNPC = new(hVampireNPC) VampireNPC(*m_pContext, m_arena, hVampireNPC, pTrueEvent);
	pVampireNPC->addDefaultComponents();

	// add the soldier as component to the ObjecManagerComponentAddon
	// all objects of this demo live in the ObjecManagerComponentAddon
	addComponent(hVampireNPC);
}
