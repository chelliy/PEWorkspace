		...
		else
		{
			if (pRealEvent->hasCustomOrientation)
			{
				...
				if (StringOps::strcmp(pRealEvent->m_meshFilename, "nazicar.x_carmesh_mesh.mesha") == 0) {
					...
					PhysicsComponent* pPC = new(hPhysicsComponent) PhysicsComponent(*m_pContext, m_arena, hPhysicsComponent, hMeshInstance, pSN, true);
					pPC->addDefaultComponents();

					pSN->m_hPhysicsComponent = hPhysicsComponent;
				}
				...
				if (StringOps::strcmp(pRealEvent->m_meshFilename, "cobbleplane.x_pplaneshape1_mesh.mesha") == 0) {
					...
					PhysicsComponent* pPC = new(hPhysicsComponent) PhysicsComponent(*m_pContext, m_arena, hPhysicsComponent, hMeshInstance, pSN, true);
					pPC->addDefaultComponents();
					Mesh* temp = hMeshInstance.getObject<MeshInstance>()->m_hAsset.getObject<Mesh>();
					//if (temp->m_AABB_Max.m_y - temp->m_AABB_Min.m_y == 0) {
					//	OutputDebugStringA("Floor Mesh has zero width\n");
					//}
					//else {
					//	OutputDebugStringA("Floor Mesh has positive or negative width\n");
					//}
					pSN->m_hPhysicsComponent = hPhysicsComponent;
				}
			}
