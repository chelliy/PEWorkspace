...
namespace PE
{
	...
	void runPhysicsThreadSingleFrame()
	{
		...
		for (unsigned int i = 0; i < PhysicsManager::Instance()->m_PhysicsComponents.m_size; i++) {
			PhysicsComponent* pPhysicsComponent = PhysicsManager::Instance()->m_PhysicsComponents[i].getObject<PhysicsComponent>();
			pPhysicsComponent->UpdatePosition();
		}

		for (unsigned int i = 0; i < PhysicsManager::Instance()->m_PhysicsComponents.m_size; i++) {
			PhysicsComponent* pPhysicsComponent = PhysicsManager::Instance()->m_PhysicsComponents[i].getObject<PhysicsComponent>();
			for (unsigned int j = 0; j < PhysicsManager::Instance()->m_PhysicsComponents.m_size; j++) {
				if (pPhysicsComponent->m_isStatic || i == j) continue;
				PhysicsComponent* pPhysicsComponent_other = PhysicsManager::Instance()->m_PhysicsComponents[j].getObject<PhysicsComponent>();
				pPhysicsComponent->OnCollision(pPhysicsComponent_other);
			}
		}
