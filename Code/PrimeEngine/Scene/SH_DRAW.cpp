...
bool pointsAreAllNotinPlaneCheck(Array<Vector3> points, Vector4 Plane) {
	bool result = true;

	for (int i = 0; i < points.m_size; i++) {
		float distance = points[i] * Vector3(Plane.m_x, Plane.m_y, Plane.m_z) + Plane.m_w;
		if (distance >= 0) {
			result = false;
			break;
		}
...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
    ...
    if (false&&pMeshCaller->m_performBoundingVolumeCulling)
    {
        ...
        for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
        {
			...
			Handle temp = pInst->getFirstParentByType<SceneNode>();
			SceneNode* tempSN = temp.getObject<SceneNode>();
			if (tempSN) {
				//Matrix4x4 worldTransform = tempSN->m_worldTransform;
				//pMeshCaller->updateAABBWorldPoints(worldTransform);
				...
				//visable = visable && !pointsAreAllNotinPlaneCheck(pMeshCaller->m_AABB_World_Points, pcam->m_frustumPlanes[0]);
				//visable = visable && !pointsAreAllNotinPlaneCheck(pMeshCaller->m_AABB_World_Points, pcam->m_frustumPlanes[1]);
				//visable = visable && !pointsAreAllNotinPlaneCheck(pMeshCaller->m_AABB_World_Points, pcam->m_frustumPlanes[2]);
				//visable = visable && !pointsAreAllNotinPlaneCheck(pMeshCaller->m_AABB_World_Points, pcam->m_frustumPlanes[3]);
				//visable = visable && !pointsAreAllNotinPlaneCheck(pMeshCaller->m_AABB_World_Points, pcam->m_frustumPlanes[4]);
				//visable = visable && !pointsAreAllNotinPlaneCheck(pMeshCaller->m_AABB_World_Points, pcam->m_frustumPlanes[5]);

				if (visable)
				{
					//DebugRenderer::Instance()->createAABBMesh(pMeshCaller, worldTransform, 0.1f);
