...
void DebugRenderer::createALine(Vector3 start, Vector3 end, float timeToLive, Vector3 color, float scale /* = 1.0f*/)
{
	...
	if (EnableDebugRendering && m_numAvailableLineLists)
	{
		...
		list.reset(1 * 2 * 6 /*pos+color*/);



		list.add(start.m_x, start.m_y, start.m_z);
		list.add(color.m_x, color.m_y, color.m_z);
		list.add(end.m_x, end.m_y, end.m_z);
		list.add(color.m_x, color.m_y, color.m_z);
...
void DebugRenderer::createAABBMesh(Mesh* mesh, Matrix4x4 worldTransfrom, float timeToLive, float scale /* = 1.0f*/)
{
	...
	if (EnableDebugRendering && m_numAvailableLineLists)
	{
		...
		list.reset(12 * 2 * 6 /*pos+color*/);

		mesh->updateAABBWorldPoints(worldTransfrom);
		...
		for(int i = 0; i<4; i++)
		{
			Matrix4x4 transform = worldTransfrom * mesh->m_AABB_Debug_Render[i];
			Vector3 pos = transform.getPos();
			Vector3 u = pos + transform.getU() * scale;
			Vector3 v = pos + transform.getV() * scale;
			Vector3 n = pos + transform.getN() * scale;
			...
			//if (i == 0) {
			//	if (pos == mesh->m_world_AABB_Min) {
			//		OutputDebugStringA("The AABB MAX MIN still same after transform multiplicaiton in different way\n");
			//	}
			//	else {
			//		OutputDebugStringA("The AABB MAX MIN not the same after transform multiplicaiton in different way\n");
			//	}
			//}

			list.add(pos.m_x, pos.m_y, pos.m_z); 
			list.add(1.f, 0, 0); 
			list.add(u.m_x, u.m_y, u.m_z); 
			list.add(1.f, 0, 0); 
			list.add(pos.m_x, pos.m_y, pos.m_z); 
			list.add(1.f, 0, 0);  
			list.add(v.m_x, v.m_y, v.m_z); 
			list.add(1.f, 0, 0);
			list.add(pos.m_x, pos.m_y, pos.m_z); 
			list.add(1.f, 0, 0); 
			list.add(n.m_x, n.m_y, n.m_z); 
			list.add(1.f, 0, 0);
		}
