...
namespace PE {
	namespace Components {
		...
		Mesh::Mesh(PE::GameContext& context, PE::MemoryArena arena, Handle hMyself)
			...
			, m_AABB_World_Points(context, arena, 8)
			, m_AABB_Corner_Points(context, arena, 8)
		{
			m_processShowEvt = true;
			m_performBoundingVolumeCulling = false;
			for (int i = 0; i < 8; i++) {
				m_AABB_World_Points.add(Vector3());
				m_AABB_Corner_Points.add(Vector3());
			}
		}
		...
		void Mesh::updateAABBWorldPoints(Matrix4x4 worldTransform) {

			float minX = FLT_MAX, minY = FLT_MAX, minZ = FLT_MAX, maxX = -FLT_MIN, maxY = -FLT_MIN, maxZ = -FLT_MIN;
			for (int i = 0; i < 8; i++) {
				m_AABB_World_Points[i] = worldTransform * m_AABB_Corner_Points[i];
				float x = m_AABB_World_Points[i].m_x;
				float y = m_AABB_World_Points[i].m_y;
				float z = m_AABB_World_Points[i].m_z;
			...
			m_world_AABB_Max = Vector3(maxX, maxY, maxZ);
			m_world_AABB_Min = Vector3(minX, minY, minZ);
		}
		...
		void Mesh::loadFromMeshCPU_needsRC(MeshCPU& mcpu, int& threadOwnershipMask)
		{
			...
			*/
			...
			Array<PrimitiveTypes::Float32> vertexPos = m_hPositionBufferCPU.getObject<PositionBufferCPU>()->m_values;
			float minX = FLT_MAX, minY = FLT_MAX, minZ = FLT_MAX, maxX = -FLT_MIN, maxY = -FLT_MIN, maxZ = -FLT_MIN;
			for (int i = 0; i < vertexPos.m_size; i += 3)
			{
				float x = vertexPos[i];
			...
			m_AABB_Debug_Render[0].setPos(Vector3(minX, minY, minZ));
			m_AABB_Debug_Render[0].setU(Vector3(maxX - minX, 0, 0));
			m_AABB_Debug_Render[0].setV(Vector3(0, maxY - minY, 0));
			m_AABB_Debug_Render[0].setN(Vector3(0, 0, maxZ - minZ));

			m_AABB_Debug_Render[1].setPos(Vector3(maxX, minY, maxZ));
			m_AABB_Debug_Render[1].setU(Vector3(minX - maxX, 0, 0));
			m_AABB_Debug_Render[1].setV(Vector3(0, maxY - minY, 0));
			m_AABB_Debug_Render[1].setN(Vector3(0, 0, minZ - maxZ));

			m_AABB_Debug_Render[2].setPos(Vector3(maxX, maxY, minZ));
			m_AABB_Debug_Render[2].setU(Vector3(minX - maxX, 0, 0));
			m_AABB_Debug_Render[2].setV(Vector3(0, minY - maxY, 0));
			m_AABB_Debug_Render[2].setN(Vector3(0, 0, maxZ - minZ));

			m_AABB_Debug_Render[3].setPos(Vector3(minX, maxY, maxZ));
			m_AABB_Debug_Render[3].setU(Vector3(maxX - minX, 0, 0));
			m_AABB_Debug_Render[3].setV(Vector3(0, minY - maxY, 0));
			m_AABB_Debug_Render[3].setN(Vector3(0, 0, minZ - maxZ));


			m_AABB_Corner_Points[0] = Vector3(minX, minY, minZ);
			m_AABB_Corner_Points[1] = Vector3(minX, minY, maxZ);
			m_AABB_Corner_Points[2] = Vector3(maxX, minY, minZ);
			m_AABB_Corner_Points[3] = Vector3(maxX, minY, maxZ);
			m_AABB_Corner_Points[4] = Vector3(minX, maxY, minZ);
			m_AABB_Corner_Points[5] = Vector3(minX, maxY, maxZ);
			m_AABB_Corner_Points[6] = Vector3(maxX, maxY, minZ);
			m_AABB_Corner_Points[7] = Vector3(maxX, maxY, maxZ);

			m_AABB_Max = Vector3(maxX, maxY, maxZ);
			m_AABB_Min = Vector3(minX, minY, minZ);
			m_world_AABB_Max = Vector3(maxX, maxY, maxZ);
			m_world_AABB_Min = Vector3(minX, minY, minZ);
		}
