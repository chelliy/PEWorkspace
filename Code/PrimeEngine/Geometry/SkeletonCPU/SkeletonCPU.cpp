	...
	Array<Matrix4x4> &res, PrimitiveTypes::UInt32 &curIndex, const Matrix4x4 &parentTransform)
{
	...
	if (nBlend >= 1)
	{
		...
		if (isHip) {
			if ((slot0.m_flags & ROOT_TO_INPLACE) > 0) {
				PEINFO("ROOT_TO_INPLACE Actived\n");
				a0 = anim0.m_frames[0][jnt.m_index];
				b0 = anim0.m_frames[0][jnt.m_index];
			}
			//if is take cover
		...
		else
		{
			...
			if (isHip) {
				if ((slot1.m_flags & ROOT_TO_INPLACE) > 0) {
					PEINFO("ROOT_TO_INPLACE Actived\n");
			...
			else
			{
				...
				if (isHip) {
					if ((slot2.m_flags & ROOT_TO_INPLACE) > 0) {
